...
#define RESET_OUT 30 //This pin outputs high and is used for the switch, only one arduino needs it but the code should be the same so the frequencies are the same among all 3 arduinos 
#define RESET_IN 23
#define DAC DAC0
...
void setup() 
{
   ...
   attachInterrupt(digitalPinToInterrupt(RESET_IN), RESET_ISR, LOW); //Call RESET_ISR when RESET_IN is low (Switch off)
}
...
void loop() 
{
  //When the pin is high (Switch is on)
  if ((digitalRead(RESET_IN) == HIGH)) 
...
void RESET_ISR() 
{
  ...
  /*
  //If want to change num of samples
